...
*/
# Version date
$RecipeInfo['CommentBoxPlus']['Version'] = '2007-12-21';
...
SDV($DiaryBoxFmt,"<div id='diary'><form class='inputform' action='\$PageUrl' method='post' onsubmit='return checkform(this);'>
    ...
    ($EnablePostCaptchaRequired ? "</td></tr><tr><th class='prompt' align='right' valign='top'><input type='hidden' name='captchakey' value='\$CaptchaKey' />
...
function HandleCommentPost($pagename, $auth) {
  global $JournalPattern,$DiaryPattern,$Author, $EnablePostCaptchaRequired;
  global $AuthFunction, $oAuthFunction, $EnablePostToAnyPage, $EnablePreventDirectives;
  $req = CBRequestArgs();
  ...
  if (@$req['author']=='') $Author = 'anon';
  if (isset($req['message'])) { $message = $req['message']; echo $message; }
  if (@$req['postto'] && $EnablePostToAnyPage==1) {
      SDV($EditRedirectFmt, $pagename);
      $pagename = MakePageName($pagename, $req['postto']);
  }
  ...
  $HandleCommentFunction($pagename, $req, $page['text']);
  HandleEdit($pagename, $auth);
  exit;
}
...
function HandleJournalPost($pagename, $req, $pagetext) {
   ...
   $entry = $time.$JPItemStartFmt.$req['text'].$JPItemEndFmt;
...
function HandleDiaryPost($pagename, $req, $pagetext) {
   ...
   $entry = $date.$DPItemStartFmt.$req['text'].$DPItemEndFmt;
   $_POST['text'] = (strstr($pagetext, $date)) ?
        str_replace($date, $entry, $pagetext) :
        "$entry\n>><<\n\n" . $pagetext;
}
...
if ($EnablePageBreaks==1) {
  ...
  # if($EnablePostAuthorRequired == 1 && $name=='') Redirect($pagename);
   ...
   $entry.= $MPItemStartFmt.$req['text'].$MPItemEndFmt;
   $order= @$req['order'];
